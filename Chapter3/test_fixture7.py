...
class TestAliens:
    def test_aliens_task(self, browser, links):
        ...
        link = links
        browser.get(link)
        browser.implicitly_wait(10)  # ждём прогрузки страницы (неявное ожидание)

        button_enter = browser.find_element(By.ID, "ember466")  # нажимаем на кнопку авторизации на сайте
        button_enter.click()

        input_email = browser.find_element(By.ID, "id_login_email")  # вводим логин
        input_email.send_keys("Your_login")

        input_password = browser.find_element(By.ID, "id_login_password")
        input_password.send_keys("Your_password")  # вводим пароль
